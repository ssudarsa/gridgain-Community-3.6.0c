...
/**
 * Demonstrates a simple way to grid-enable a spring bean.
 * <p>
 * String "Hello World" is set into {@link GridifySpringBeanHelloWorld} instance
 * as its state. Since method {@link GridifySpringBeanHelloWorld#sayIt()} is annotated
 * with {@code @Gridify} annotation it is automatically grid-enabled and is
 * executed on remote grid node. The {@link GridifySpringBeanHelloWorld} is
 * initialized in {@code "spring-bean.xml"} Spring configuration file.
 * <p>
    ...
    public static void main(String[] args) throws GridException {
        ...
        try {
            ...
            AbstractBeanFactory factory = new XmlBeanFactory(
                new ClassPathResource("org/gridgain/examples/helloworld/gridify/springbean/spring-bean.xml"));

            // Get the Spring bean to grid-enable.
            GridifySpringBeanHelloWorld bean = (GridifySpringBeanHelloWorld)factory.getBean("hello.world.bean");

            // This method will be executed on a remote grid nodes.
            int phraseLen = bean.sayIt();

            X.println(">>>");
            X.println(">>> Finished executing Gridify Spring Bean \"Hello World\" example.");
