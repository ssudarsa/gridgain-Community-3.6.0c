 ...
 * saved within session with {@link GridTaskSessionScope#SESSION_SCOPE} scope
 * will be removed from checkpoint storage. Checkpoints saved with
 * {@link GridTaskSessionScope#GLOBAL_SCOPE} will outlive the session and
 ...
 * To see example on how to use {@code GridTaskSession} refer to
...
public interface GridTaskSession extends GridMetadataAware {
     ...
     * when using <tt>continuous mapping</tt> (see {@link GridTaskContinuousMapper}),
     ...
     * <p>
     * This method defaults checkpoint scope to {@link GridTaskSessionScope#SESSION_SCOPE} and
     * implementation will automatically remove the checkpoint at the end of the session. It is
     * analogous to calling {@link #saveCheckpoint(String, Object, GridTaskSessionScope, long)
     ...
     * If {@link GridTaskSessionScope#GLOBAL_SCOPE} is used, the checkpoint will outlive
     ...
     * @param key Key to be used to load this checkpoint in future.
     * @param state Intermediate job state to save.
     * @param scope Checkpoint scope. If equal to {@link GridTaskSessionScope#SESSION_SCOPE}, then
     *      state will automatically be removed at the end of task execution. Otherwise, if scope is
     *      {@link GridTaskSessionScope#GLOBAL_SCOPE} then state will outlive its session and can be
