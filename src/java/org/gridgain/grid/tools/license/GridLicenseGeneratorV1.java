 ...
 */
public class GridLicenseGeneratorV1 {
    /** Property key-value separator. */
    private static final char SEPARATOR = '=';

    /** Default license version. */
    private static final String LIC_VER = "1.0";

    /** Default issue organization. */
    private static final String ISSUE_ORG = "GridGain Systems";
    ...
    static {
        ...
        c.setPatterns(new String[] {"MM/dd/yyyy"});
    ...
    public static void main(String[] args) throws GridException {
        ...
        GridLicenseV1 lic = buildLicense(argMap);
     ...
     * Ensures singleton.
     */
    private GridLicenseGeneratorV1() {
        // No-op.
    }
     ...
     */
    private static void saveLicense(GridLicenseV1 lic, Map<String, String> args) throws GridException {
        assert lic != null;
        assert args != null;
        ...
        try {
            Marshaller marsh = JAXBContext.newInstance(GridLicenseV1Adapter.class).createMarshaller();
     ...
     */
    private static GridLicenseV1 buildLicense(Map<String, String> args) throws GridException {
        assert args != null;

        GridLicenseV1Adapter lic = new GridLicenseV1Adapter();
     ...
     */
    private static void setUserArguments(GridLicenseV1Adapter lic, Map<String, String> args) throws GridException {
        assert lic != null;
        assert args != null;
     ...
     * @throws GridException If property set failed.
     */
    private static void setSystemProperties(GridLicenseV1Adapter lic) throws GridException {
        assert lic != null;

        lic.setId(UUID.randomUUID());
        lic.setVersion(LIC_VER);
        lic.setIssueOrganization(ISSUE_ORG);

        if (lic.getIssueDate() == null)
            lic.setIssueDate(new Date());

        try {
            lic.setSignature(GridLicenseUtil.createSignatureV1(getKey(), lic));
        }
    ...
    private static Map<String, String> getArgumentsMap(String[] args) throws GridException {
        ...
        for (String s : args) {
            ...
            if (idx <= 0)
                throw new GridException("Invalid key-value argument in: " + s);
            ...
            if (val.isEmpty())
                throw new GridException("Argument value can't be empty in: " + s);
