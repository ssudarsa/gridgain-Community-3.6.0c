...
public abstract class GridOutClosure<T> extends GridLambdaAdapter implements Callable<T>, GridJob {
     ...
     * @return New closure.
     */
    public <A> GridOutClosure<A> andThen(final GridClosure<T, A> c) {
        return new GridOutClosure<A>() {
            @Override public A apply() {
                return c.apply(GridOutClosure.this.apply());
            }
        };
    }
     ...
     * @return New closure.
     */
    public GridAbsClosure andThen(final GridInClosure<T> c) {
        return new GridAbsClosure() {
            @Override public void apply() {
                c.apply(GridOutClosure.this.apply());
            }
        };
    }
}
