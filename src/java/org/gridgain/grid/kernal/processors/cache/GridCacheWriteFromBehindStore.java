...
/**
 * Internal wrapper for a {@link GridCacheStore} that enables write-from-behind logic.
 * <p/>
 ...
 * <p/>
 * The essential flush size for the write-from-behind cache should be at least the estimated
...
public class GridCacheWriteFromBehindStore<K, V> implements GridCacheStore<K, V> {
    ...
    private int cacheMaxSize = GridCacheConfiguration.DFLT_WRITE_FROM_BEHIND_FLUSH_SIZE;
    ...
    private long cacheFlushFreq = GridCacheConfiguration.DFLT_WRITE_FROM_BEHIND_FLUSH_FREQUENCY;

    /** Maximum batch size for put and remove operations */
    private int batchSize = GridCacheConfiguration.DFLT_WRITE_FROM_BEHIND_BATCH_SIZE;

    /** Name of the cache that this store will serve. */
    ...
    /** Write cache. */
    private GridConcurrentLinkedHashMap<K, StatefulValue<V>> writeCache;
    ...
    /**
     * Gets the maximum size of the write-from-behind buffer. When the count of unique keys
     ...
     * {@link GridCacheConfiguration#DFLT_WRITE_FROM_BEHIND_CRITICAL_SIZE}
     *
     ...
     */
    public GridCacheWriteFromBehindStore(String gridName, GridLogger log, GridCacheStore<K, V> store,
    ...
    public void start() throws GridException {
        ...
        if (stopping.compareAndSet(true, false)) {
            if (log.isDebugEnabled())
                log.debug("Starting write-from-behind store for cache '" + cacheName + '\'');

            cacheCriticalSize = (int)(cacheMaxSize * CACHE_OVERFLOW_RATIO);

            if (cacheCriticalSize == 0)
                cacheCriticalSize = GridCacheConfiguration.DFLT_WRITE_FROM_BEHIND_CRITICAL_SIZE;
     ...
     */
    public int getWriteFromBehindTotalCriticalOverflowCount() {
    ...
    public void stop() throws GridException {
        if (stopping.compareAndSet(false, true)) {
            if (log.isDebugEnabled())
                log.debug("Stopping write-from-behind store for cache '" + cacheName + '\'');
     ...
     */
    @Override public void loadAll(@Nullable String cacheName, GridInClosure2<K, V> c, @Nullable Object... args)
        throws GridException {
        store.loadAll(cacheName, c, args);
    }

    /** {@inheritDoc} */
    @SuppressWarnings({"NullableProblems"})
    @Override public void loadAll(@Nullable String cacheName, @Nullable GridCacheTx tx,
        @Nullable Collection<? extends K> keys, GridInClosure2<K, V> c) throws GridException {
        if (log.isDebugEnabled())
            log.debug("Store load all [keys=" + keys + ", tx=" + tx + ']');

        Collection<K> remaining = new LinkedList<K>();
        ...
        if (!remaining.isEmpty())
            store.loadAll(cacheName, null, remaining, c);
    }
    ...
    @Override public V load(@Nullable String cacheName, @Nullable GridCacheTx tx, K key) throws GridException {
        ...
        return store.load(cacheName, null, key);
    }

    /** {@inheritDoc} */
    @Override public void putAll(@Nullable String cacheName, @Nullable GridCacheTx tx,
    ...
    /** {@inheritDoc} */
    @Override public void removeAll(@Nullable String cacheName, @Nullable GridCacheTx tx,
    ...
    @Override public String toString() {
        return S.toString(GridCacheWriteFromBehindStore.class, this);
    }
    ...
    private void updateCache(K key, @Nullable V val, StoreOperation operation) throws GridInterruptedException {
        StatefulValue<V> newVal = new StatefulValue<V>(val, operation);
    ...
    private void applyBatch(Map<K, StatefulValue<V>> valMap) {
        ...
        Map<K, V> batch = new HashMap<K, V>(valMap.size());
    ...
    private boolean updateStore(StoreOperation operation, Map<K, V> vals) {
        try {
            switch (operation) {
                case PUT:
                    store.putAll(cacheName, null, vals);

                    break;

                case RMV:
                    store.removeAll(cacheName, null, vals.keySet());
    ...
    private class Flusher extends GridWorker {
        ...
        private void flushCache(Iterator<Map.Entry<K,StatefulValue<V>>> it) {
            ...
            Map<K, StatefulValue<V>> batch = null;
            Map<K, StatefulValue<V>> pending  = new LinkedHashMap<K, StatefulValue<V>>(batchSize);
            ...
            while (it.hasNext()) {
                ...
                try {
                    ...
                    if (operation != val.operation()) {
                        ...
                        batch = pending;
                        pending = new LinkedHashMap<K, StatefulValue<V>>(batchSize);
