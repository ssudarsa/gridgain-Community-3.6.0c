...
public class GridCacheQueryIndex<K, V> {
    ...
    private ThreadLocal<ConnectionWrapper> connCache = new ThreadLocal<ConnectionWrapper>() {
        ...
        @Nullable @Override protected ConnectionWrapper initialValue() {
            ...
            try {
                String user = cctx.config().getIndexUsername();
                String password = cctx.config().getIndexPassword();
     ...
     */
    GridCacheQueryIndex(GridCacheContext<K, V> cctx) {
        assert cctx != null;
    ...
    void start() throws GridException {
        ...
        String dbName = cctx.nodeId().toString() + "_" + cctx.namex();
     ...
     */
    private void schemaWriteUnlock() {
    ...
    private void bindKey(PreparedStatement stmt, int idx, K key, @Nullable byte[] keyBytes, TableDescriptor table)
        ...
        if (cctx.config().isIndexFixedTyping() && dbKeyType != DBTypeEnum.BINARY) {
            ...
            if (keyBytes != null)
                ...
                try {
                    U.marshal(cctx.marshaller(), key, out);
    ...
    private void bindValue(PreparedStatement stmt, int idx, V val, TableDescriptor table) throws GridException {
        ...
        try {
            if (DBTypeEnum.fromClass(val.getClass()) == DBTypeEnum.BINARY) {
                ...
                try {
                    U.marshal(cctx.marshaller(), val, out);
    ...
    public void store(K key, @Nullable byte[] keyBytes, V val, GridCacheVersion ver) throws GridException {
        ...
        if (log.isDebugEnabled())
            log.debug("Storing key to cache query index [locId=" + cctx.nodeId() + ", key=" + key +
                ", value=" + val + "]");
        ...
        try {
            ...
            if (log.isDebugEnabled())
                log.debug("Updated rows in query index [locId=" + cctx.nodeId() + ", updated=" + updated + ']');
        }
        catch (SQLException e) {
    ...
    public boolean remove(K key, @Nullable byte[] keyBytes) throws GridException {
        ...
        if (log.isDebugEnabled())
            log.debug("Removing key from cache query index [locId=" + cctx.nodeId() + ", key=" + key + ']');
        ...
        try {
            for (TableDescriptor table : tables) {
                if (!cctx.config().isIndexFixedTyping() || table.type().keyClass().equals(key.getClass())) {
                    ...
                    if (updated > 0) {
                        if (log.isDebugEnabled())
                            log.debug("Removed index for key [locId=" + cctx.nodeId() + ", key=" + key +
                                ", updated=" + updated + ']');
    ...
    Iterator<GridCacheQueryIndexRow<K, V>> querySql(GridCacheQueryBaseAdapter query, boolean loc) throws GridException {
        ...
        ClassLoader ldr = cctx.deploy().isGlobalLoader() || !loc ?
            cctx.deploy().globalLoader() : cctx.deploy().localLoader();
            ...
            if (!F.isEmpty(query.arguments())) {
                ...
                for (Object arg : query.arguments()) {
                    if (!arg.getClass().isArray() && DBTypeEnum.fromClass(arg.getClass()) == DBTypeEnum.BINARY) {
                        ...
                        try {
                            U.marshal(cctx.marshaller(), arg, out);
                        }
                        catch (GridException e) {
    ...
    private void createTable(TableDescriptor table, Statement stmt) throws SQLException {
        ...
        String keyType = cctx.config().isIndexFixedTyping() ? dbTypeFromClass(table.type().keyClass()) : "BINARY";
    ...
    private class TableDescriptor {
        ...
        TableDescriptor(QueryType type) {
            ...
            tableName = (cctx.config().isIndexFullClassName()) ?
    ...
    private class AnalyzeWorker extends GridWorker {
        ...
        protected AnalyzeWorker() {
            super(cctx.gridName(), ANALYZE_THREAD_NAME, log);
        }
    ...
    private class ResultSetIterator implements Iterator<GridCacheQueryIndexRow<K, V>> {
        ...
        private GridCacheQueryIndexRow<K, V> loadRow() throws SQLException, GridException {
            ...
            if (cctx.config().isIndexFixedTyping() && rs.getMetaData().getColumnType(1) != Types.VARBINARY)
