...
public abstract class GridCacheAdapter<K, V> extends GridMetadataAwareAdapter implements GridCache<K, V>,
    ...
    private static final ThreadLocal<GridTuple2<String, String>> stash = new ThreadLocal<GridTuple2<String,
        String>>() {
    ...
    @Override public void clearAll(@Nullable GridPredicate<? super GridCacheEntry<K, V>>[] filter) {
        ...
        for (GridCacheEntryEx<K, V> e : map.entries0()) {
            if (obsoleteVer == null)
                    ...
                    return new GridEmbeddedFuture<Map<K, V>, Map<K, V>>(
                        ...
                        new C2<Map<K, V>, Exception, GridFuture<Map<K, V>>>() {
                            @Override public GridFuture<Map<K, V>> apply(Map<K, V> map, Exception e) {
                                ...
                                return new GridFinishedFuture<Map<K, V>>(ctx.kernalContext(), Collections.<K,
                                    V>emptyMap());
                            }
                        },
    ...
    public boolean txLock(Collection<? extends K> keys, long timeout, GridCacheTxLocalEx<K, V> tx, boolean invalidate,
        ...
        return F.isEmpty(keys) || txLockAsync(keys, timeout, tx, isRead, retval, isolation, invalidate, CU.<K,
            V>empty()).
            get();
    }
    ...
    @Override public int partition(K key) {
        return configuration().getAffinity().partition(key);
    }
    ...
    @Override public int[] allPartitions(GridProjection p) {
        ...
        for (int part = 0; part < aff.partitions(); part++)
            for (GridRichNode n : aff.nodes(part, CU.allNodes(ctx)))
                if (p.contains(n)) {
                    parts.add(part);
    ...
    @Override public GridRichNode mapPartitionToNode(int part) {
        GridCacheAffinity<Object> aff = configuration().getAffinity();

        A.ensure(part >= 0 && part < aff.partitions(), "part >= 0 && part < total partitions");

        return F.first(aff.nodes(part, CU.allNodes(ctx)));
    }

    /** {@inheritDoc} */
    @Override public Map<Integer, GridRichNode> mapPartitionsToNodes(int part, int[] parts) {
        Map<Integer, GridRichNode> map = new HashMap<Integer, GridRichNode>();

        map.put(part, mapPartitionToNode(part));
