...
public class GridEmailProcessor extends GridProcessorAdapter {
    ...
    @Override public void start() throws GridException {
        if (isSmtpEnabled) {
            ...
            q = new LinkedList<GridEmailHolder>();

            sender = new GridThread(ctx.config().getGridName(), "email-sender-thread", worker);
    ...
    public GridFuture<Boolean> schedule(String subj, String body, boolean html) {
        ...
        return addrs == null || addrs.length == 0 ? new GridFinishedFuture<Boolean>(ctx, false) :
