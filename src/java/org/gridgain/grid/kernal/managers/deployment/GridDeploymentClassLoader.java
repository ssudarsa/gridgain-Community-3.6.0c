...
class GridDeploymentClassLoader extends ClassLoader implements GridDeploymentInfo {
    ...
    void register(UUID nodeId, GridUuid ldrId, long seqNum) {
        ...
        synchronized (mux) {
            ...
            nodeLdrMap.remove(nodeId);

            // 2. Save pointer.
            Map<UUID, GridTuple2<GridUuid, Long>> map = nodeLdrMap;

            // 3. Create new map.
            nodeLdrMap = new LinkedHashMap<UUID, GridTuple2<GridUuid, Long>>(map.size() + 1, 0.75f, true);

            // 4. Put passed in first.
            nodeLdrMap.put(nodeId, F.t(ldrId, seqNum));

            // 5. Put all other nodes.
            nodeLdrMap.putAll(map);
        }
    }
    ...
    private GridByteArrayList sendClassRequest(String name, String path) throws ClassNotFoundException {
        ...
        if (log.isDebugEnabled())
            log.debug("Loader map entries: " + entries);
            ...
            try {
                ...
                if (log.isDebugEnabled())
                    log.debug("Failed to find class on remote node [class=" + name + ", nodeId=" + node.id() +
                        ", clsLdrId=" + entry.getValue() + ", reason=" + res.errorMessage() + ']');
                ...
                throw new ClassNotFoundException("Failed to peer load class [class=" + name + ", nodeClsLdrs=" +
                    entries + ", parentClsLoader=" + getParent() + ", reason=" + res.errorMessage() + ']');
            }
        ...
        for (Map.Entry<UUID, GridTuple2<GridUuid, Long>> entry : entries) {
            ...
            try {
                ...
                if (res == null) {
                    ...
                    return new ByteArrayInputStream(res.byteSource().getInternalArray(), 0,
                        res.byteSource().getSize());
                }
            }
            catch (GridException e) {
                // This thread should be interrupted again in communication if it
