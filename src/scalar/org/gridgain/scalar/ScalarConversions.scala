...
trait ScalarConversions {
    ...
    /**
     * Implicit converter from `Callable` to Scala wrapping function.
     *
     * @param f Grid closure to convert.
     */
    implicit def fromCallable[R](f: Callable[R]): () => R =
        new ScalarCallableFunction[R](f)

    /**
     * Implicit converter from `Runnable` to Scala wrapping function.
     *
     * @param f Grid closure to convert.
     */
    implicit def fromRunnable(f: Runnable): () => Unit =
        new ScalarRunnableFunction(f)
    ...
    /**
      * Pimp for adding explicit conversion method `scala`.
      *
      * @param f Java-side callable to pimp.
      */
    implicit def callableDotScala[R](f: Callable[R]) = new {
        def scala: () => R =
            fromCallable(f)
    }
    ...
    /**
      ...
      * @param f Java-side callable to pimp.
      */
    implicit def runnableDotScala(f: Runnable) = new {
        def scala: () => Unit =
            fromRunnable(f)
    }
